...
def send_to_blynk(pin, value):
    """Send update to Blynk (queued, async)."""
    if not USE_BLYNK:
        return
...
while True:
    ...
    cv2.putText(frame,"LIGHT",(w//4-40,h//4),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),2)
    cv2.putText(frame,"FAN",(3*w//4-30,h//4),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),2)
    cv2.putText(frame,"BED BULB",(w//4-70,3*h//4),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),2)
    cv2.putText(frame,"AC",(3*w//4-30,3*h//4),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),2)
    ...
    if result.multi_hand_landmarks:
        ...
        if len(gesture_history)>BUFFER_SIZE:
        ...
        if now - last_time > cooldown:
            ...
            if stable >= 4:
                ...
                if current_led_zone != zone:
                    ...
                    for z,pin in ZONE_LED_PINS.items():
                        send_to_blynk(pin, 0)
    ...
    # ---------------- DISPLAY ----------------
    if last_output:
        cv2.putText(frame,last_output,(30,h-30),
    ...
    if highlight_time and time.time()-highlight_time < 1:
        ...
        if len(parts)>1:
            ...
            highlight(frame, zone_name, color)
